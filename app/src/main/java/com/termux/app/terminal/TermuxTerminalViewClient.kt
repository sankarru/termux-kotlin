...
open class TermuxTerminalViewClient(
    @JvmField internal val mActivity: TermuxActivity,
    @JvmField internal val mTermuxTerminalSessionActivityClient: TermuxTerminalSessionActivityClient
) : TermuxTerminalViewClientBase() {

    /** Keeping track of the special keys acting as Ctrl and Fn for the soft keyboard and other hardware keys. */
    @JvmField internal var mVirtualControlKeyDown = false
    @JvmField internal var mVirtualFnKeyDown = false
    ...
    override fun onSingleTapUp(e: MotionEvent?) {
        if (e == null) return
        val currentSession = mActivity.getCurrentSession() ?: return
        val term = currentSession.emulator ?: return
    ...
    override fun isTerminalViewSelected(): Boolean {
        return mActivity.isTerminalViewSelected() || mActivity.getTerminalView().hasFocus()
    }

    override fun copyModeChanged(copyMode: Boolean) {
        // Disable drawer gestures while copying.
        mActivity.setDrawerGesturesEnabled(!copyMode)
    }

    @SuppressLint("RtlHardcoded")
    override fun onKeyDown(keyCode: Int, e: KeyEvent?, session: TerminalSession?): Boolean {
        if (e == null || session == null) return false
        ...
        if (keyCode == KeyEvent.KEYCODE_ENTER && !session.isRunning) {
            ...
            } else if (unicodeChar in '1'.code..'9'.code) {
    ...
    fun readExtraKeysSpecialButton(specialButton: SpecialButton?): Boolean {
        ...
        val state = mActivity.getExtraKeysView().readSpecialButton(specialButton, true)
        if (state == null) {
            Logger.logError(
                LOG_TAG,
    ...
    private fun setSessionShortcuts() {
        ...
        for (entry in TermuxPropertyConstants.MAP_SESSION_SHORTCUTS.entries) {
            ...
            if (codePoint != null) {
                mSessionShortcuts?.add(KeyboardShortcut(codePoint, entry.value))
            }
        }
    }
    ...
    fun setSoftKeyboardState(isStartup: Boolean, isReloadTermuxProperties: Boolean) {
        ...
        mActivity.getTerminalView().onFocusChangeListener = View.OnFocusChangeListener { _, hasFocus ->
            ...
            val terminalToolbarCompose = mActivity.findViewById<View>(R.id.activity_termux_compose)
            if (terminalToolbarCompose != null) {
                textInputViewHasFocus = terminalToolbarCompose.hasFocus() && mActivity.isTerminalToolbarTextInputViewSelected()
            }
    ...
    private fun getShowSoftKeyboardRunnable(): Runnable {
        var runnable = mShowSoftKeyboardRunnable
        if (runnable == null) {
            runnable = Runnable {
                KeyboardUtils.showSoftKeyboard(mActivity, mActivity.getTerminalView())
            }
    ...
    fun setTerminalCursorBlinkerState(start: Boolean) {
        if (start) {
            ...
            if (mActivity.getTerminalView().setTerminalCursorBlinkerRate(mActivity.getProperties().terminalCursorBlinkRate)) {
    ...
    fun shareSessionTranscript() {
        ...
        val rawTranscript = ShellUtils.getTerminalSessionTranscriptText(session, false, true) ?: return

        // See https://github.com/termux/termux-app/issues/1166.
        val transcriptText = DataUtils.getTruncatedCommandOutput(
            rawTranscript,
